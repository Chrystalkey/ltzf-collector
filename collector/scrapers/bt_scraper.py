...
class BundestagAPIScraper(Scraper):
    ...
    async def listing_page_extractor(self, url) -> list[str]:
        ...
        async with self.session.get(endpoint, params=params) as response:
            if response.status == 200:
                ...
                num = data["numFound"]
                logger.info(
                    f"{num} Vorgang gefunden für Zeitraum {startdatum} - {enddatum}"
                )
                ...
                logger.debug(
                    f"Error while fetching listing page: {await response.text()}"
                )
                return []
        ...
        gsvh = models.Vorgang.from_dict(
            {
                ...
                "verfassungsaendernd": "Änderung des Grundgesetzes"
                in vorgang.get("titel", ""),
                "trojaner": False,
                ...
                "ids": [
                    models.VgIdent.from_dict(
                        {"typ": "vorgnr", "id": str(vorgang.get("id"))}
                    ),
                    models.VgIdent.from_dict(
                        {
                            "typ": "initdrucks",
    ...
    async def _get_vorgangspositionen(self, vorgang_id: str) -> List[Dict]:
        ...
        async with self.session.get(endpoint, params=params) as response:
            if response.status == 200:
                ...
                logger.debug(
                    f"Error while fetching vorgangspositionen: {await response.text()}"
                )
                return []
    ...
    def _extract_initiatoren(self, vorgang: Dict) -> List[Dict]:
        ...
        if vorgang.get("initiative"):
            for initiator in vorgang["initiative"]:
                autor = models.Autor.from_dict({"organisation": initiator}).to_dict()
    ...
    def _extract_autoren(self, position: Dict) -> List[Dict]:
        ...
        if position.get("fundstelle", {}).get("urheber", {}):
            for urheber in position["fundstelle"]["urheber"]:
                autor = models.Autor.from_dict({"organisation": urheber}).to_dict()
    ...
    async def _create_station_from_position(
        self, position: Dict
        ...
        if typ == "Abstimmung":
            beschluss = position.get("beschlussfassung", [{}])[0].get(
                "beschlusstenor", ""
            )
            ...
            if not typ and beschluss.startswith(
        ...
        if not datum:
            logger.warning(
                f"Kein Datum gefunden für Position {position.get('titel', '')}, verwende aktuelles Datum"
            )
    ...
    async def _extract_dokumente(
        self, position: Dict, typ: models.Stationstyp
    ) -> List[Dict]:
        ...
        logger.info(
            f"Dokument: {drsnr}, {btapi_doctyp}, Zusammenfassung: {zusammenfassung}"
        )
    ...
    def _parse_date(self, date_str: str) -> str:
        ...
        logger.warning(
            f"Konnte Datum '{date_str}' nicht parsen, verwende aktuelles Datum"
        )
        return datetime.now().astimezone().isoformat()
