...
def test_documents():
    ...
    assert returned.to_json() == mock_dok.to_json(), "Retrieved Document did not match stored one"
    raw_ret = cache.get_raw("dok:blub")
    assert raw_ret is not None, "Expected Raw Key to be dok:blub, but was unable to retrieve under that name"
...
def test_vorgang():
    ...
    mock_vg = models.Vorgang.from_dict({
        ...
        "initiatoren": [models.Autor.from_dict({"person": "Peter Zwegat", "organisation": "Die Linke"})],
        "stationen": [models.Station.from_dict({
            "titel": "Testtitelstation",
            "zp_start": datetime.datetime.now().astimezone(datetime.UTC),
            "zp_modifiziert": datetime.datetime.now().astimezone(datetime.UTC),
            "parlament": "BB",
            "typ": "preparl-regent",
            "trojanergefahr": 4,
            "dokumente": [],
    ...
    assert raw_ret is not None, "Expected Raw Key to be vg:blub, but was unable to retrieve under that name"
...
def test_website():
    ...
    assert raw_ret is not None, "Expected Raw Key to be html:blub, but was unable to retrieve under that name"