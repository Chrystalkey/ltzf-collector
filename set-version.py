...
replace(
    ...
    r"tags: crystalkey/ltzf-backend:\d+\.\d+\.\d+",
    f"tags: crystalkey/ltzf-backend:{version}",
)
...
replace(
    ...
    f'ENV OPENAPI_GENERATOR_VERSION="{oapid["oapigen-version"]}"',
)
replace(
    "oapigen.ps1",
    r"$OPENAPI_GENERATOR_VERSION=\"\d+\.\d+\.\d+\"",
    f'$OPENAPI_GENERATOR_VERSION="{oapid["oapigen-version"]}"',
)
replace(
    "oapigen.sh",
    r"OPENAPI_GENERATOR_VERSION=\"\d+\.\d+\.\d+\"",
...
replace(
    ...
    f'ENV SPEC_PATH="{oapid["oapi-spec"]}"',
)
replace(
    "oapigen.ps1", r"\$SPEC_PATH=\"https://.*\"", f'$SPEC_PATH="{oapid["oapi-spec"]}"'
)
